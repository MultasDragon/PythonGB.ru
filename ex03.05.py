...
def poiskator2(YA_STROKA, YA_SUMMATOR = 0):    
    YA_STROKA = YA_STROKA.replace(",",".")    
    YA_SUMM = 0.0
    while "  " in YA_STROKA:
        YA_STROKA = YA_STROKA.replace("  "," ")    
    ...
    for i in YA_list:            
        if i.replace(".","").replace("-","").replace("+","").isdigit():
            ...
            while i[j].isdigit():
                j += 1
            YA_SUMM += float(i[:j]) if j>0 else 0
            if i[j].upper() == "N":
                print(f'В итоге сумма ({YA_SUMM:.1f}){YA_SUMMATOR + YA_SUMM:.2f}')
                return
            YA_SUMMATOR += YA_SUMM
            return poiskator2(input(f'Текущая сумма ({YA_SUMM:.1f}){YA_SUMMATOR:.3f}, продолжим ввод? (N - прекращаем) '), YA_SUMMATOR)
    YA_SUMMATOR += YA_SUMM
    return poiskator2(input(f'Текущая сумма ({YA_SUMM:.1f}){YA_SUMMATOR:.3f}, продолжим ввод? (N - прекращаем) '), YA_SUMMATOR)
    

poiskator2(input('Введите числа, разделенные пробелом, при вводе буквы "N", алгоритм прекратит алгоритмить: '))
